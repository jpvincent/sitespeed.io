...
class StorageManager {
  ...
  createDirForUrl(url, subDir) {
    return Promise.resolve(this.getDirForUrl(url, subDir))
      .tap((dirPath) => mkdirp(dirPath));
  }

  writeDataForUrl(data, filename, url, subDir) {
    const dirPath = this.getSubDirPath(subDir)
    return write(this.createDirForUrl(url, dirPath), filename, data);
  }

  getSubDirPath (subDir) {
    return ['data', subDir].filter(Boolean).join(path.sep)
  }
  getDirForUrl (url, subDir) {
    return [
      this.baseDir,
